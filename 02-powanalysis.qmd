...
```
```{r}
power_graph <- counties |>
  left_join(power_totals, by = c("GEOID" = "fips"))
```

```{r}
ggplot(power_graph) +
  geom_sf(aes(fill = total_outages), color = "black")
```
...
```{r}
power_states <- power_cat |>
  group_by(state_event) |>
  summarize(total_events = n())
