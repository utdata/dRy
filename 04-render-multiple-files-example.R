...
taylor_reports <- tibble(                    # creating a dataframe
  ...
  output_file = str_glue("{album}.html"),    # column called output_file that is named based on the album we create the report for
  execute_params = map(album, ~list(album = .))
)
...
# The tilda is a shorthand way to show that we are using a lambda function. Without the tilda it would look like this: function (x) list(album = x)
